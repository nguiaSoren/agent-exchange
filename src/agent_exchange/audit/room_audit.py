...
async def _signal_complete(
    on_member_complete: Callable[[str, list[Finding]], Awaitable[None]] | None,
    specialty: str,
    findings: list[Finding],
) -> None:
    """Fire the per-member completion callback with the member's findings, containing failure.

    Progress is purely informational (it drives the live UI's per-agent "done" beat AND lets
    the caller surface the member's REAL findings in the room as it finishes), so a raising
    callback must never abort the member's round or lose its findings. Failures are logged at
    WARNING and swallowed.
    """
    if on_member_complete is None:
        return
    try:
        await on_member_complete(specialty, findings)
...
async def _member_round(
    ...
    on_member_complete: Callable[[str, list[Finding]], Awaitable[None]] | None = None,
) -> list[Finding]:
    ...
    async with sem:
        try:
            ...
            return []
        await _signal_complete(on_member_complete, member.specialty, findings)
...
async def collaborate_in_room(
    ...
    *,
    ...
    on_member_complete:
        Optional async callback fired with ``(member.specialty, findings)`` the moment that
        member's in-room audit (its ``findings()``) resolves — whether it returned findings or
        raised (then ``findings`` is ``[]``, still "completed"). Fires once per member,
        concurrently, as each really finishes, so a live caller can stream per-agent progress
        AND the member's real findings into the room while this coroutine is still running. A
